...
- Frozen training snapshot: `.local/six_d/current/raw/continuation/final_training_snapshot.csv` (SHA in `health_metrics.json`).
- Training GP: production Matérn-5/2 ARD(6) with `-ln(LLHD)` score, `Standardize(m=1)`, `train_Yvar = 1/LLHD²` — same recipe as the 4D validated workflow.
- All five plots come from `optimize/bayesian/workflows/six_d/finalize.py`.
- Held-out points and repeatability checks are in `.local/six_d/current/raw/direct_validation/heldout_6d_results.csv` and `reference_points.csv`; those points were never used to fit the GP shown here.

## 01_convergence_and_efficiency.png

- Top panel plots every training LLHD by ordered evaluation index, colored by phase (INITIAL, BO_1, DIRECT, INTERACTION, BO_2). Dashed verticals mark phase boundaries.
...
- What the plot answers: did BO_1 improve upon its INITIAL sampling? Did BO_2 improve upon the pre-continuation best? How close is the final training best to the all-nominal held-out reference?
- What the plot cannot prove: convergence of BO_2 to the true simulator minimum. A flat running-best only means BO_2 did not sample a lower-loss point — the direct-profile plot must be used to check whether one exists.
...
- Panel A: 5-fold cross-validated predicted score vs actual for every training row. Panel B: held-out simulator observations (12 global + 12 local Sobol points around final best), with 1-sigma vertical bars. Panel C: standardized held-out residuals `(pred − obs)/SD`.
- The 24 held-out points were never used to fit the GP shown here. CV is an interpolation check; held-out is the honest test.
- CV RMSE(score) = 0.1129 vs constant-mean baseline 1.0475. Held-out RMSE(score) = 0.1145 vs baseline 0.8912. Held-out Spearman ρ = 0.988. Held-out coverage 68% / 95% = 0.92 / 0.96.
